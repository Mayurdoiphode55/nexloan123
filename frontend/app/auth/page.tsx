...
export default function AuthPage() {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await verifyOTP({ identifier: otpIdentifier, otp });
      storeToken(res.data.access_token);
      storeUser(res.data.user);
      setSuccess('OTP verified! Redirecting...');
      setTimeout(() => router.push('/dashboard'), 1200);
    } catch (err: any) {
      const d = err.response?.data?.detail; setError(typeof d === 'string' ? d : Array.isArray(d) ? d.map((e: any) => e.msg || JSON.stringify(e)).join(', ') : err.message || 'OTP verification failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      const d = err.response?.data?.detail; setError(typeof d === 'string' ? d : Array.isArray(d) ? d.map((e: any) => e.msg || JSON.stringify(e)).join(', ') : err.message || 'Failed to resend OTP');
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="auth-card__fields">
                  ...
                  <div className="animate-stagger" style={{ '--stagger-index': 2 } as React.CSSProperties}>
                    <Input
                      ...
                      onChange={(val) => handleInputChange('mobile', val.replace(/\D/g, '').slice(0, 10))}
                      maxLength={10}
                      required
                    />
                  </div>
                ...
                <div className="auth-card__fields">
                  <div className="animate-stagger" style={{ '--stagger-index': 0 } as React.CSSProperties}>
                    <Input
                      ...
                      onChange={(val) => { setLoginIdentifier(val); setError(''); }}
                      required
                    />
                  </div>
                </div>
